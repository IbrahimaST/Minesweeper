...
class Cell {
  boolean hasMine; // indicates if the cell contains a mine
  boolean isRevealed; // indicates if the cell is revealed
  boolean hasFlag; // indicates if the cell has a flag
  int neighboringMines; // the number of neighboring cells with mines
...
class MinesweeperGame extends World {
  ...
  ArrayList<ArrayList<Cell>> board; // the game board
  Random randomObj; // object for generating random numbers
  WorldScene lastScene; // the final scene of the game
  ...
  public void initBoard() {
    ...
    while (minePositions.size() < numMines) {
      int randomRow = randomObj.nextInt(rows);
      int randomColumn = randomObj.nextInt(cols);
  ...
  public void updateGameStatus() {
    ...
    if (hasLost) {
      ...
      endOfWorld("Congratulations! You win!");
    }
  }
...
class ExamplesMines {
  ...
  public void testMinesweeperGame(Tester t) {
    ...
    t.checkExpect(game.lastScene, game.lastScene("Congratulations! You win!"));
    ...
    game.updateGameStatus();
    t.checkExpect(game.lastScene, game.lastScene("Game Over! :("));
